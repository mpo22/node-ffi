...
var Type = require('./type')
  ...
  , FFI_STDCALL = bindings.FFI_STDCALL
  , FFI_FIRST_ABI = bindings.FFI_FIRST_ABI
  , FFI_LAST_ABI = bindings.FFI_LAST_ABI
  // status codes
...
function CIF (rtype, types, abi) {
  ...
  if (typeof abi === 'undefined' || abi === 0) {
    debug('no ABI specified (this is OK), using FFI_DEFAULT_ABI')
    abi = FFI_DEFAULT_ABI
  } else if (process.platform === 'win32' && abi == FFI_STDCALL) {
    if (process.arch === 'x64') {
        abi = FFI_DEFAULT_ABI
    }
  } else {
    if (!(abi > FFI_FIRST_ABI && abi < FFI_LAST_ABI)) {
      var err = new Error('FFI_BAD_ABI')
